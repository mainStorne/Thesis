...
from src.grpc.quota_pb2 import CreateStudentRequest, UploadUserAppRequest, UploadUserAppResponse
...
class QuotaServicer(QuotaServiceServicer):
    ...
    @scoped([get_session, authorize, get_quota_service])
    async def CreateStudent(self, request: CreateStudentRequest, context, quota_service: QuotaService, **state):
        try:
            await quota_service.create_student(request)
        except QuotaError:
            return await context.abort(StatusCode.INTERNAL, 'Quota error')
