...
from src.api.deps import AuthStudentDependency, SessionDependency
...
async def create_project(request: Request, project_name: str, auth: AuthStudentDependency, session: SessionDependency, template_id: UUID, queue_token: str):
    ...
    queue_var.set(queue)
    try:
        student_project = await project_service.create_student_project(project_name, student.account, buffer, session, template_id, filesize)
