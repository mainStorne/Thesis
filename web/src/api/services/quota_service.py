...
from web.src.api.services.base import ServiceError

from src.api.db.users import Account
from src.api.repos.archive_repo import ArchiveRepo, IArchiveRepo
from src.api.repos.docker_repo import docker_repo
from src.api.repos.quota_repo import QuotaRepo
...
class QuotaService:
    ...
    async def upload_user_app(self, buffer: BytesIO, buffer_size: int, filename: str):
        ...
        if user_df["BlockUsed"] + buffer_size > user_df["BlockHardLimit"]:
            raise QuotaExcitedError
        path = settings.quota.students_home_base_dir / self._user_account.login / filename
        try:
            archived = await self._archive_repo.create_tar(
                path, buffer, self._user_account.login, self._user_account.login
            )
        except Exception as e:
            raise ArchiveError from e
        # filename is `file.zip` in tag this name need to be `file`
        container = await docker_repo.build(archived, f"{self._user_account.login}-{filename[:4]}")
...
async def get_quota_service(state: dict):
    ...
    state["quota_service"] = QuotaService(user_account, quota_repo, ArchiveRepo())
    yield
